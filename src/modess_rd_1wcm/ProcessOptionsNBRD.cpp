...
NCPA::ProcessOptionsNB::ProcessOptionsNB(AnyOption *opt) {
  ...
  // First, file type
  enum AtmosphericFileType { ATMOSFILE, G2SENVFILE, ASCIIDIR, JETFILE, SLICEFILE};   // Expand this enum as we put in more file types
  ...
  if (numTypesDeclared != 1) {
      delete opt;
      throw invalid_argument( "Choose only one source for the atmospheric profiles: options --use_1D_profiles_from_dir or --g2senvfile or --atmosfile.");
  }
  ...
  //cout << "in ProcessOptionsNBRD: opt->getValue( ""use_profile_ranges_km"" ) = " << opt->getValue( "use_profile_ranges_km" ) << endl;
  
  //cout << "in ProcessOptionsNBRD: profile_ranges_given = " << profile_ranges_given << endl;
...
} // ------- End of constructor processOptions ---------------------------------
