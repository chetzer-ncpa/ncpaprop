                    ...
                    double **mode_S, double **mode_R)
{
  ...
  //for(i=0;i*df<f_vec[0];i++) arg_vec[i]=0.0; // left zero pad - original
  for(i=0;i*df<f_vec[0]/2;i++) arg_vec[i]=0.0; // in effect make only the first element zero
  ...
  for(i=0;i<n_freqs;i++){
      ...
      //// Transfer function * df i.e.
      //// pressure * df (see eq. 5.14 pg 274 in Oc. Acoust. 1994 ed.);
      //// at short distance from source this is exp(I*k0*r)/(4*Pi*r)
      //arg_vec[i0+i]=df*sqrt(I/(8.0*Pi*range))*cup; 

      cup=cup*dft_vec[i];  // multiply by the source spectrum
      //arg_vec[i0+i]=df*range*sqrt(2.0*I)*cup/(sqrt(Pi*range)); // Roger's original scaling

      // compute (pressure*source_spectrum*df) (see eq. 5.14 pg 274 in Oc. Acoust. 1994 ed.)
      // !!! note that "cup" already has the factor df=f_step via dft_vec passed into here from pulse_prop();      
      
      // the reduced pressure is defined as: (actual pressure)/sqrt(rho(z)): p_red(r,z) = p(r,z)/sqrt(rho(z))
      // the next line is the reduced pressure times sqrt(rho(zsrc)); compare with eq. 24 in DV's Modess notes page 3  
      //arg_vec[i0+i] = expov8pir*cup; 
      
      //// use the next line for actual physical pressure containing sqrt_rho_ratio
      arg_vec[i0+i] = expov8pir*cup*sqrt_rho_ratio;
  }

  smooth_space=(int)floor(0.1*n_freqs); // smoothly zero out on right; from RW code I got in July 2012
...
/*
// obsolete - debug version of 'fft_pulse_prop'
void fft_pulse_prop(double t0, int n_freqs, double df, double *f_vec, \
                    double range, complex<double> *dft_vec, \
                    complex<double> *pulse_vec, \
                    int *mode_count, double rho_zsrc, double rho_zrcv, \
                    ...
                    double **mode_S, double **mode_R)
{
  ...
  for(i=0;i<n_freqs;i++){
      ...
      cup=cup*dft_vec[i];  // multiply by the source spectrum; 'dft_vec' already has the df factor in it
      //arg_vec[i0+i]=df*range*sqrt(2.0*I)*cup/(sqrt(Pi*range)); // Roger's original scaling
      ...
      //// use the next line for actual physical pressure containing sqrt_rho_ratio
      arg_vec[i0+i] = expov8pir*cup*sqrt_rho_ratio; // pressure at frequency f_vec[i0+i]
  }
  ...
  // 
  //perform fft of arg_vec to obtain the time domain pulse
  //
  p=fftw_plan_dft_1d(FFTN, reinterpret_cast<fftw_complex*>(arg_vec), \
												   reinterpret_cast<fftw_complex*>(pulse_vec), \
												   FFTW_FORWARD,FFTW_ESTIMATE); 												 
...
/*
// version that uses only the built-in "Roger" pulse
								...
								double **mode_S, double **mode_R) 
{
  ...
  arg_vec   = new complex<double> [FFTN];
  // max_speed=2.0*PI*f_vec[n_freqs-1]/re_k[n_freqs-1][mode_count[n_freqs-1]];

  // save the Fourier Transform of the initial pulse: see 'cup'
  f=fopen("ft_initial_pulse.dat","w");
  for(i=0; i<n_freqs; i++) {
      cup=pulse_spec_fit(scale,f_vec[i]);
      fprintf(f,"%14.9f %15.6e %15.6e\n", f_vec[i], real(cup), imag(cup));
      dft_vec[i]=f_step*pulse_spec_fit(scale,f_vec[i])*exp(I*2.0*Pi*f_vec[i]*scale); // source spectrum
      arg_vec[i]=dft_vec[i]; // FFTN length vector storing dft_vec in the first n_freqs points
      // uncomment below if you want the hack to output the transfer function 
      // and not the pulse; we set the spectrum to that of a delta function i.e all ones 
      // dft_vec[i] = 1.0;      
  }
  fclose(f);
  cout << "Fourier transform of initial pulse saved in file 'ft_initial_pulse.dat' " << endl
       << " with format: | Freq (Hz) | Re(ft) | Imag(ft) |" << endl;

  for( ; i<FFTN; i++) arg_vec[i]=0.0; //zero the rest of arg_vec[] up to FFTN
  ...
  f = fopen("initial_pulse.dat","w");
  for(i=0;i<(FFTN*f_step*5*scale);i++) {
    	fprintf(f,"%12.6f %15.6e %15.6e\n", 1.0*i/fmx, real(pulse_vec[i]), imag(pulse_vec[i]));
  }
  fclose(f);
  cout << "Initial pulse saved in file 'initial_pulse.dat' " << endl
       << " with format: | Time (s) | Re(pulse) | Imag(pulse) |" << endl;
       
	//
	// fft propagation: 'pulse_vec' is rewritten with the propagated pulse
	//							
  ...
  if (1) {
      ...
      plotXY(xData, yData, FFTN, RR/1000.0);
      delete [] xData;
      delete [] yData;
  }	
...
// 20130708 - new version of pulse_prop that allows loading of a custom source pulse
int pulse_prop(	const char *filename,double t0,double RR,  \
								...
								complex<double> *arg_vec, int src_flg, string srcfile) 
{
  ...
  if (src_flg==0) { // use spectrum of a delta function => to get impulse response; 
      ...
      if ((f_center > 0) & (f_center <= f_vec[n_freqs-1]/5.0+1.0E-16)) {	
          ...
          std::ostringstream es;
          es << "for the builtin pulse f_center should be set <= f_max/5 = " 
             << f_vec[n_freqs-1]/5.0;
          throw invalid_argument(es.str());
      }
      ...
      load_source_spectrum(srcfile, dft_vec, FFTN);
  }

  if ((src_flg<3)) { // common block only if the source spectrum is available
      for(i=0; i<n_freqs; i++) arg_vec[i] = dft_vec[i]*f_step;
      for( ; i<FFTN; i++) arg_vec[i]=0.0; //arg_vec[] is the source spectrum zero padded up to FFTN
      ...
      for (i=0; i<n_freqs; i++) {
          dft_vec[i] = arg_vec[i]*dt; // factor dt needed 
      }
  }
  else { 
  ...
  if (1) {
      ...
      if (src_flg==1) {
        for(i=0;i<(FFTN*f_step*5*scale);i++) {
          	//fprintf(f,"%12.6f %15.6e %15.6e\n", 1.0*i/fmx, real(pulse_vec[i]), imag(pulse_vec[i]));
          	// save 2*real(pulse_vec[i]); factor of 2 because we only use the real part
...
/*
// old function - made obsolete in July 2013
int pulse_prop_src2rcv_grid(\
          const char *filename,double max_cel, \
          double R_start,double DR,double R_end, \
					int n_freqs, double f_step, double *f_vec, \
					...
					double **re_k, double **im_k, double **mode_S, double **mode_R) 
{
  ...
  fftw_execute(p);
  fftw_destroy_plan(p);
  //cout <<"in pulse_prop before initial_pulse.dat: fstep = " << f_step << "\n";
  fmx = ((double)FFTN)*f_step; 
  f = fopen("initial_pulse.dat","w");
  for(i=0;i<(FFTN*f_step*5*scale);i++) {
      fprintf(f,"%12.6f %15.6e %15.6f\n", 1.0*i/fmx, real(pulse_vec[i]), imag(pulse_vec[i]));
  }
  fclose(f);

  printf("----------------------------------------------\n");
  printf("max_celerity     t0         R_km\n");
  printf("----------------------------------------------\n");
...
int load_source_spectrum(string srcspfn, complex<double> *dft_vec, int fftn) {
  // load source spectrum
  cout << "Loading time domain source pulse from file " << srcspfn << endl;
