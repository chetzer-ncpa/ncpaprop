...
int main( int argc, char **argv ) {
  ...
  if (param->getBool( "dispersion" )) { //compute and write dispersion
      ...
      // make sure dispersion file will not be overwritten
      // we should hide this and initialize the file before starting to append to it
      ...
      ModeSolver *solver;
      /*
      double f_min, f_step, f_max, freq;
      ...
      // sanity checks
      if (f_min >= f_max) {
        cerr << "f_min must be less than f_max!" << endl;
        return 1;
      }

      param->addParameter( new NCPA::FloatParameter( "freq", 0.0 ) );
      FloatParameter *freqparam = (FloatParameter *)(param->getParameter("freq"));
      */

      //for (freq = f_min; freq <= f_max; freq += f_step) {
      //  freqparam->setValue( freq );
    ...
    BroadbandPropagator *prop = new BroadbandPropagator( param );
    prop->calculate();
    delete prop;
