							...
							double **mode_S, double **mode_R)
{
  ...
  while(fscanf(fp,"%lf",&f_vec[n])==1){
      ...
      fscanf(fp,"%lf",prho_zsrc);
      fscanf(fp,"%lf",prho_zrcv);
...
// DV 20170805 - Added the second pulse choice from Roger Waxler's updated broadband code
...
//
int get_source_spectrum( int n_freqs, int NFFT, double f_step, double *f_vec, double f_center, \
                         ...
                         complex<double> *arg_vec, int src_flg, string srcfile) 
{
  ...
  if (src_flg==0) { // use spectrum of a delta function => to get the impulse response;
  ...
  else if ( (src_flg==1) || (src_flg==2) ) { //use one of the two built-in pulses 
                                             // (provided by Roger Waxler)
      ...
      if (src_flg==1) {  
          ...
          for(i=0; i<n_freqs; i++) {
              ...
              //dft_vec[i] = pulse_spec_fit(scale,f_vec[i])*exp(I*2.0*Pi*f_vec[i]*scale);
              //dft_vec[i] = dft_vec[i]*exp(I*2.0*Pi*f_vec[i]*5.0*scale/4.0); // DV 20150720 - time delay of '5.0*scale/4.0' added;
          }

          // the next 'if' contains engineering code - for developing/debugging purposes 
          if (0) {
              ...
              //
              // perform fft on arg_vec to obtain the pulse at the source (time domain) of NFFT points: 'pulse_vec'
              //
              ...
              // temporarily save pulse choice #1
              if (0) {
                f = fopen("pulse_input1.dat","w");
                //for(i=0;i<(NFFT*f_step*5*scale);i++) {
                for(i=0;i<NFFT;i++) {
                  fprintf(f,"%12.6f %15.6e\n", 1.0*i/fmx, real(pulse_vec[i])); 
                }      
                fclose(f);
                cout << "--> Initial source waveform saved in file " 
                     << "'pulse_input1.dat' " << endl
                     << " with format: | Time (s) | Amplitude |" << endl;
              }


              // temporary saving dft_vec to file
              if (0) {

              f=fopen("builtin_source_spectrum_choice1.dat","w"); // save the source spectrum in this file
              for(i=0; i<n_freqs; i++) {
                  fprintf(f,"%14.9f %15.6e %15.6e\n", f_vec[i], real(dft_vec[i]), imag(dft_vec[i]));    
              }
              fclose(f);
              cout << "--> The built-in source spectrum (choice #1) is saved in file "
                   << "'builtin_source_spectrum_choice1.dat' " << endl
      ...
      if (src_flg==2) {
          ...
          model_pulse_fft(power, scale, dt, NFFT, arg_vec); // arg_vec is the FT of initial pulse choice #2
          for (i=0; i<n_freqs; i++) {
              dft_vec[i] = arg_vec[i]; //  multiplication by the dt factor already done in model_pulse_fft()
              //printf("dft[%d] = %g + %g i\n", i, real(dft_vec[i]), imag(dft_vec[i]));
          }

          if (0) { // save initial pulse spectrum
          f=fopen("ft_initial_pulse_choice2.dat","w");
          for(i=0;i<n_freqs;i++){
            dft_vec[i]=arg_vec[i];
            fprintf(f,"%f   %g   %g\n",f_vec[i], real(dft_vec[i]), imag(dft_vec[i]));
          }
          ...
          if (0) { // save initial pulse spectrum
          f=fopen("ft_initial_arg_vec_choice2.dat","w");
          for(i=0;i<NFFT;i++){
          ...
          if (0) { // obtain the initial pulse: from dft_vec to time domain pulse_vec
              ...
              // save the intial pulse choice #2 
              if (1) {
                
                f = fopen("initial_builtin_pulse_choice2.dat","w");
                //for(i=0;i<(NFFT*f_step*5*scale);i++) {
                for(i=0;i<NFFT;i++) {
                  fprintf(f,"%12.6f %15.6e\n", 1.0*i/fmx, real(pulse_vec[i])); 
                }      
                fclose(f);
                cout << "--> Initial source pulse #2 saved in file " 
                     << " 'initial_builtin_pulse_choice2.dat' " << endl
                     << " with format: | Time (s) | Amplitude |" << endl;
              }
          }
      } // end pulse choice # 2   
  ...
  else if (src_flg==3) { //use custom source spectrum from a file
  ...
  // obtain the initial pulse at the source to save in file "source_waveform_input.dat"
  // either from builtin or user-provided source spectra
  if ((src_flg<4)) { // common block only if the source spectrum (at positive frequencies) 
                     // is available: either the builtin spectra or loaded from a file
  ...
  else if (src_flg==4) { // use custom source pulse (time domain) from a file
                         // and obtain its spectrum (at 0 and positive frequencies)
      double dt;
      vector<double> t, tdp;
  ...
  if (1) {
      ...
      if ( (src_flg==1) || (src_flg==2) ) { // the builtin pulse choices 1 and 2
        double factor = 2.0;
        //for(i=0;i<(NFFT*f_step*5*scale);i++) {
        for(i=0;i<NFFT/2;i++) {
        ...
        double factor = 2;
        if (src_flg==4) factor = 1;  // use custom source pulse (time domain) from a file
...
// this function kept here because it is still called by 
// the part of the code doing 2D propagation; that part is disabled for now; DV 20170811
// the source spectrum is loaded/computed in this function
                    ...
                    double **mode_S, double **mode_R)
{
  ...
  //printf("!!! in fft_pulse_prop: sqrt_rho_ratio = %g\n", sqrt_rho_ratio);
  ...
  for(i=0;i*df<0.99*f_vec[0];i++) arg_vec[i]=0.0; // left zero pad
  i0 = i;
  for(i=0;i<n_freqs;i++){
  ...
  if (0) {
      FILE *fp = fopen("modbb_arg_vec.dat", "w");
      for (i=0; i<NFFT; i++) {
          fprintf(fp, "%12.9f %15.6e %15.6e\n", 1.0*i*df, real(arg_vec[i]), imag(arg_vec[i]));
      }
      fclose(fp);
      printf("arg_vec saved in file 'modbb_arg_vec.dat' with format: i*df | Re(arg_vec) | Im(arg_vec)\n");

      //for(int n=0; n<5; n++) {
      //  printf("k[n][0]=%g\n", re_k[n][0]);
      //}

      fp = fopen("modbb_dft_vec.dat", "w");
      for (i=0; i<n_freqs; i++) {
          fprintf(fp, "%12.9f %15.6e %15.6e\n", 1.0*i*df, real(dft_vec[i]), imag(dft_vec[i]));
      }
      fclose(fp);
      printf("dft_vec saved in file 'modbb_dft_vec.dat' with format: i*df | Re(dft_vec) | Im(dft_vec)\n");
  }
  ...
  get_source_spectrum(n_freqs, NFFT, f_step, f_vec,  f_center, \
	...
	if (pprop_src2rcv_flg) { // propagation to one receiver at distance RR from source
      ...
      cout << "--> Propagated pulse saved in file: '" << filename << "'" << endl
           << "' with format: | Time (s) | Re(pulse) |" << endl;  
...
// New pulse_spec_fit function from Roger Waxler new broadband code; DV 20170811
...
// Old pulse fit function - commented out DV 20170811
//complex<double> pulse_spec_fit(double scale, double x) { 
...
void model_pulse_fft(double power,double scale, double dt, int NFFT, complex<double> *dft_vec) {
  int i;
  complex<double> *p_vec;
  fftw_plan p;

  p_vec=(complex<double> *)malloc(sizeof(complex<double>)*NFFT);
  ...
  for(i=0;i<NFFT;i++){
    p_vec[i]=model_pulse_shape(power,scale,i*dt); // the pulse timeseries
      ...
      // DV - temporarily save the pulse timeseries
      if (0) {
        printf(" ---> in model_pulse_fft 1a\n");
        cout << "nfft=" << NFFT << endl;
        FILE * f = fopen("modbb_builtin_pulse2.dat","w");
        for(i=0;i<NFFT;i++) {
          fprintf(f,"%12.6f %15.6e\n", i*dt, real(p_vec[i])); 
        }      
        fclose(f);
        printf( "--> Initial source pulse #2 saved in file \n" ); 
        printf( " 'modbb_builtin_pulse2.dat' \n" );
        printf( " with format: | Time (s) | Amplitude |\n" );
      }
...
complex<double> ***getPz1z2 (int I1, int I2, double r1, double r2, double dr, string dirn, list<string> files, bool zero_attn_flg) 
{
  int lenI, ier;
  int ir, j, jj, iz;
  int Nfreq, Nz_grid, Nz_subgrid, nmods;
  //double NN;
  ...
  ier = ier + 1; // useless line just to avoid warning by the compiler that 'ier' is not used
  //printf(" ... Pfzr done.\n");
  return Pfzr;
}
