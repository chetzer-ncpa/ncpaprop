...
/*
 * This is Jelle Assink's Complex Normal Modes code effective sound speed- moved to C++
...
int main( int argc, char **argv ) {
  ...
  //if (oNB->getPlot_flg()) {
      ...
      ////plotwGNUplot(freq, write_2D_TLoss, Nby2Dprop, write_phase_speeds);
      //plotwGNUplot(oNB);   
  //}
...
AnyOption *parseInputOptions( int argc, char **argv ) {
  ...
  opt->addUsage( "|                          Complex Normal Modes                            |" );
  ...
  opt->addUsage( "   file tloss_1d.cnm - considering attenuation in the atmosphere" );
  opt->addUsage( "   file tloss_1d.lossless.cnm  - no attenuation" );
	opt->addUsage( "Additionally, if the flag --write_2D_TLoss is present on the command line" );
	opt->addUsage( "the 2D TL is saved to file tloss2d.cnm" );  
  ...
  opt->addUsage( "" );
  opt->addUsage( "The options below can be specified in a colon-separated file \"CModess.options\"" );
  ...
  opt->addUsage( "                          The output in saved into default files: " );  
  opt->addUsage( "                              Nby2D_tloss_1d.lossless.cnm" );
  opt->addUsage( "                              Nby2D_tloss_1d.cnm" );
  opt->addUsage( " --write_atm_profile      Save the interpolated atm. profile to" );
  opt->addUsage( "                          default file: atm_profile.cnm" );
  ...
  opt->addUsage( "  tloss_1d.cnm:              r, 4*PI*Re(P), 4*PI*Im(P), (incoherent TL)" );
  opt->addUsage( "  tloss_1d.lossless.cnm:" );
  opt->addUsage( "  tloss_2d.cnm:              r, z, 4*PI*Re(P), 4*PI*Im(P)" );   
  opt->addUsage( "  Nby2D_tloss_1d.cnm:        r, theta, 4*PI*Re(P), 4*PI*Im(P), (incoherent TL)" );
  opt->addUsage( "  Nby2D_tloss_1d.lossless.cnm:" );
  opt->addUsage( "" );
  opt->addUsage( "  phasespeeds.cnm:           Mode#, phase speed [m/s], imag(k)" );
  opt->addUsage( "  mode_<mode_count>.cnm      z, (Mode amplitude)" );
  opt->addUsage( "  dispersion_<freq>.cnm      Contains one line with entries:" );
  opt->addUsage( "                             freq, (# of modes), rho(z_src)," );  
  opt->addUsage( "                             followed for each mode 'i' by (Re, Im) parts of: " );
  opt->addUsage( "                             (k(i)) Mode(i)(z_src) Mode(i)(z_rcv)" );
  opt->addUsage( "  atm_profile.cnm            z,u,v,w,t,d,p,c,c_eff" );  
  opt->addUsage( "" );
  
  opt->addUsage( " Examples (run from 'samples' directory): " );
  opt->addUsage( "    ../bin/CModess --atmosfile NCPA_canonical_profile_zuvwtdp.dat --atmosfileorder zuvwtdp --skiplines 0 --azimuth 90 --freq 0.1" );
  opt->addUsage( "" );  
  ...
  opt->processFile( "./CModess.options" );
