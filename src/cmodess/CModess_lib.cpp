...
int plotwGNUplot(double freq, bool write_2D_TLoss, bool Nby2Dprop, bool write_phase_speeds) {
  ...
  if (!Nby2Dprop) {  
      ...
      fprintf(pipe, "set title 'CModess: Transmission loss; Frequency %g Hz'\n", freq);
      fprintf(pipe, "set xlabel 'Range [km]'\n");
      fprintf(pipe, "set ylabel 'TL [dB]'\n");
      fprintf(pipe, "set grid; show grid;\n");           
      fprintf(pipe, "plot './tloss_1d.lossless.cnm' using 1:(10*log10($2**2 + $3**2)) with lines lt 3 title 'lossless' , \\\n");
      fprintf(pipe, "    './tloss_1d.cnm' using 1:(10*log10($2**2 + $3**2)) with lines lt 1 title 'lossy'\n");        
      pclose(pipe);
  }
  ...
  if (write_2D_TLoss & (!Nby2Dprop)) {
      ...
      fprintf(pipe, "set title 'CModess: Transmission loss; Frequency %g Hz'\n", freq);      
      fprintf(pipe,"splot './tloss_2d.cnm' using 1:2:(20*log10(sqrt($3**2 + $4**2)))\n");
      pclose(pipe);
  }
  ...
  if (Nby2Dprop) {
      ...
      fprintf(pipe, "set title 'CModess: Transmission loss; Frequency %g Hz'\n", freq);      
      fprintf(pipe,"splot './Nby2D_tloss_1d.cnm' using ($1*(sin($2*pi/180))):($1*(cos($2*pi/180))):(10*log10($3**2 + $4**2)) title ''\n");
      pclose(pipe);        
  ...
  if (write_phase_speeds) {
      ...
      //fprintf(pipe, "set data style lines\n");
      //fprintf(pipe, "set yrange [%f:%f]\n",min(alt,nAlt),max(alt,nAlt));       
      fprintf(pipe, "set title 'CModess: Phase speeds; Frequency %g Hz'\n", freq);
      fprintf(pipe, "set xlabel 'Mode #'\n");
      fprintf(pipe, "set ylabel 'm/s'\n");
      fprintf(pipe, "set grid; show grid;\n");
      fprintf(pipe, "plot './phasespeeds.cnm' using 1:2 with lines; \\\n");      
      pclose(pipe);
  }     
  return 0;
}
...
int saveAtm_profile(NCPA::SampledProfile *p) {
  ...
  FILE *fp = fopen("atm_profile.cnm", "w");
  for (i=0; i<nz; i++) {
      z = p->z(i);
  ...
  fclose(fp);
  printf("Interpolated atmospheric profiles (z,u,v,w,t,d,p,c,c_eff) saved in: atm_profile.cnm\n");
  return 0;
}
*/
