...
AnyOption::getValue( const char *option )
{
	...
	cout << "Looking for " << option << endl;
	for( int i = 0 ; i < option_counter ; i++ ){
		if( strcmp( options[i], option ) == 0 ) {
			cout << "Returning " << option << " = " << values[ optionindex[i] ] << endl;
...
AnyOption::processFile()
{
	if( ! (valueStoreOK() && FileSet())  )
		return false;
	cout << "Using " << filename << " to read" << endl;
	consumeFile( readFile() );
	cout << "Now have " << g_value_counter << " values" << endl;
	return true;
