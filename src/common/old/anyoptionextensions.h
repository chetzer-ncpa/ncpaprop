...
namespace NCPA {
	...
	public:
		...
		void addFlag( std::string key );
		void addIntegerOption( std::string key );
		void addIntegerOption( std::string key, int default_value );
		void addStringOption( std::string key );
		void addStringOption( std::string key, std::string default_value );
		void addFloatOption( std::string key );
		void addFloatOption( std::string key, double default_value );
		...
		void addUsageLine( std::string line );
		void printUsage();
		
		void setCommandLineArguments( int _argc, char **_argv );
		void setOptionFileName( std::string filename );
		void commandLineOverRidesFile( bool tf );
		void getOptions();
		...
		/**
		  ...
		  * @param out The string to which to print them.
		  */
		void printTestDescriptions( std::ostream *out ) const;
		
	private:
		
		AnyOption *_parser;
		
		std::map< std::string, OPTION_TYPE > _typemap;
		std::map< std::string, bool > _boolmap;
		std::map< std::string, int > _intmap;
		std::map< std::string, double > _floatmap;
		...
		bool _useArgs, _useFile, _argsOverrideFile;
