...
# PETSC_OS = linux-gnu
# PETSC_ARCH_REAL = arch-darwin-c-real
# PETSC_ARCH_COMPLEX = arch-darwin-c-complex
...
#modess_rd_2wcm:
#	$(MAKE) -C src/modess_rd_2wcm ModessRD2WCM PETSC_ARCH=arch-darwin-c-complex

# clean everything up, including external libraries
clean:
	-$(MAKE) -C src/common clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/atmosphere clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/raytrace clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/modess clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/modbb clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/modess_rd_1wcm clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/pade_pe clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/wmod clean  PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/cmodess clean  PETSC_ARCH=arch-darwin-c-complex
	-$(MAKE) -C src/cmodbb clean  PETSC_ARCH=arch-darwin-c-complex
	-$(MAKE) -C src/wnlrt clean PETSC_ARCH=arch-darwin-c-real
	-$(MAKE) -C src/tdpape clean PETSC_ARCH=arch-darwin-c-real
#	-$(MAKE) -C src/modess_rd_2wcm clean  PETSC_ARCH=arch-darwin-c-complex
	#-$(MAKE) -C src/extern clean  PETSC_ARCH=arch-darwin-c-real
